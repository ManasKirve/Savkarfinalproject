...
def _convert_keys_to_camel_case(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, value in data.items():
        ...
        if isinstance(value, dict):
            ...
            converted[camelKey] = [_convert_keys_to_camel_case(item) if isinstance(item, dict) else item for item in value]
        else:
...
# Legal Notice functions - Using implementation from the second code block
...
def create_notice_for_user(uid: str, notice_data: Dict[str, Any]) -> LegalNotice:
    ...
    try:
        ...
        # Ensure borrower_id is stored as a string
        if 'borrower_id' in notice_data:
            notice_data['borrower_id'] = str(notice_data['borrower_id'])
        
        logger.info(f"Creating notice with data: {notice_data}")
