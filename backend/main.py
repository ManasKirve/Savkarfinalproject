...
# Legal Notices - Updated to work like loans (no UID required)
@app.get("/notices", response_model=List[LegalNotice])
def get_all_notices():
    try:
        # Get notices for the savkar user (just like loans)
        savkar_user_id = firestore_repo.SAVKAR_USER_ID
        notices_data = firestore_repo.get_notices_for_user(savkar_user_id)
        return notices_data
    except Exception as e:
...
def create_notice(notice: NoticeCreate):
    try:
        # Create notice for the savkar user (just like loans)
        savkar_user_id = firestore_repo.SAVKAR_USER_ID
        # Use dict(by_alias=False) to get snake_case field names for Firestore
        data = notice.dict(by_alias=False)
        saved = firestore_repo.create_notice_for_user(savkar_user_id, data)
        return saved
    except Exception as e:
...
def update_notice(notice_id: str, notice_update: NoticeUpdate):
    try:
        # Update notice for the savkar user (just like loans)
        savkar_user_id = firestore_repo.SAVKAR_USER_ID
        # Use dict(by_alias=False) to get snake_case field names for Firestore
        update_data = notice_update.dict(by_alias=False, exclude_unset=True)
        updated = firestore_repo.update_notice_for_user(savkar_user_id, notice_id, update_data)
        return updated
    except Exception as e:
...
def delete_notice(notice_id: str):
    try:
        # Delete notice for the savkar user (just like loans)
        savkar_user_id = firestore_repo.SAVKAR_USER_ID
        firestore_repo.delete_notice_for_user(savkar_user_id, notice_id)
